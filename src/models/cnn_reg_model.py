...
class MAPFDataset(Dataset):
    def __init__(self, df, runtime_cols, transforms=transforms.Compose([]), datafolder='custom_mapf_images'):
        self.df = df
    ...
    def __getitem__(self, idx):
        ...
        x = transforms.Normalize(mean=[0, 0, 0],
                                 std=[1, 1, 1])(x)
        y = torch.tensor(row[self.runtime_cols])
        return x, y
...
class CNNMapfRegression(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        a, y_hat = torch.min(y_hat, dim=1)
        acc = accuracy_score(y.clone().cpu().detach().numpy().argmin(axis=1), y_hat.clone().cpu().detach().numpy())
    ...
    def test_step(self, batch, batch_nb):
        ...
        acc = accuracy_score(y.clone().cpu().detach().numpy().argmin(axis=1), y_hat.cpu())
...
class CNNRegModel(MapfModel):
    ...
    def train_cv(self, data, labels, exp_type, load=False, model_suffix='regression-model.pt',
                 models_dir='models/cnn-regression/', n_splits=1):
        ...
        for index, (tr_ind, test_ind) in enumerate(gkf.split(data[self.features_cols], labels, groups)):
            print("Starting {i} inner fold out of {n} in cnn regression training".format(i=index, n=n_splits))
            ...
            self.model = CNNMapfRegression(val_df=data.iloc[test_ind].copy(),
            ...
            if load and os.path.exists(curr_model_path):
                ...
                self.model.eval()
                print("loaded cnn regression model from", curr_model_path)
                return
            train = MAPFDataset(data.iloc[tr_ind].copy(), runtime_cols=self.only_alg_runtime_cols)
            val = MAPFDataset(data.iloc[test_ind].copy(), runtime_cols=self.only_alg_runtime_cols)
    ...
    def predict(self, X_test, y_test, online_feature_extraction_time=None):
        test_dataset = MAPFDataset(X_test, runtime_cols=self.only_alg_runtime_cols)
