...
class MAPFDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = transforms.Normalize(mean=[0, 0, 0],
                                 std=[1, 1, 1])(x)
        y = torch.tensor(row[self.success_cols]).float()
        return x, y
...
class CNNMapfCoverage(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        output = F.sigmoid(output)  # We use sigmoid since we want a classifier for each algorithm's coverage
        return output
    ...
    def validation_step(self, batch, batch_nb):
        ...
        loss = criterion(y_hat, y)
        # acc = accuracy_score(y.clone().cpu().detach().numpy(), y_hat.cpu())
        p = torch.argmax(y_hat.cpu(), dim=1)
...
class CNNCoverageModel(MapfModel):
    def __init__(self, *args):
        super(CNNCoverageModel, self).__init__(*args)
        self.modelname = 'CNN Coverage'
        self.model = None
        if self.maptype != '':
    ...
    def train_cv(self, data, labels, exp_type, load=False, model_suffix='coverage-model.pt',
                 models_dir='models/cnn-coverage/', n_splits=1):
        ...
        for index, (tr_ind, test_ind) in enumerate(gkf.split(data[self.features_cols], labels, groups)):
            ...
            print("Starting {i} inner fold out of {n} in cnn coverage training".format(i=index, n=n_splits))
            self.model = CNNMapfCoverage(val_df=data.iloc[test_ind].copy(),
            ...
            if load and os.path.exists(curr_model_path):
                ...
                self.model.eval()
                print("loaded cnn coverage model from", curr_model_path)
                return
    ...
    def predict(self, X_test, y_test, online_feature_extraction_time=None):
        ...
        y_test = X_test['Y']
        test_preds = self.model.test_preds.cpu().detach().numpy().argmax(axis=1)
